...
// 加载 glTF 资源
gltfLoader.load(
  'model/Duck.glb',  // glb二进制模型数据， gltf json模型数据
  (gltf) => {
...
const dracoLoader = new DRACOLoader(); // 创建解码器
...
gltfLoader.load(
  'model/city.glb',  // glb二进制模型数据， gltf json模型数据
  (gltf) => {
    scene.add(gltf.scene)
  },
  (xhr) => {
...
rgbeLoader.loadAsync('texture/Alex_Hart-Nature_Lab_Bones_2k.hdr').then((envMap) => {
  envMap.mapping = threeJs.EquirectangularReflectionMapping; // 设置映射类型
  scene.environment = envMap; // 设置场景的环境贴图
  // scene.background = texture; // 设置场景的背景贴图
})
